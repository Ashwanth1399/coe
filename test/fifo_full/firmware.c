...
#define fifo_wr_en (*(volatile uint32_t*)0x03000004)
#define fifo2_rd_en (*(volatile uint32_t*)0x03000008)
//#define fifo_empty_flag (*(volatile uint32_t*)0x0300001C)
...
void read_from_fifo() {
    int offset = 0; // Initialize offset for SRAM writes
    uint32_t fifo_data; // Variable to hold data read from FIFO
    cpu_status = 1;
    cpu_status_change = 1;

    // Wait until TB_status becomes 1
    while (TB_status != 1) {
    ...
    TB_status = 0;
    TB_status_change = 0;
    // Read data from FIFO and write it to SRAM
    for (int i = 1; i < 9; i++) {
        fifo2_rd_en = i;
